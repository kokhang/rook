...
// NewServer creates a new api rook server but does not configure it
func NewServer(api *operations.RookAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the rook API
type Server struct {
	EnabledListeners []string `long:"scheme" description:"the listeners to enable, this can be repeated and defaults to the schemes in the swagger spec"`

	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/rook.sock"`
...
func (s *Server) SetAPI(api *operations.RookAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving rook at unix://%s", s.SocketPath)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving rook at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving rook at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
